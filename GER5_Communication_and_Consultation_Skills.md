...
> [!note] **MERGED INTO `[[Communication]]` — C1, 2026-09-01.**
> All six sections (`§0.1` Consultation structure · `§0.2` Breaking Bad News · `§0.3` Difficult
> Conversations · `§0.4` Interpreters · `§0.5` Shared Decision-Making · `§0.6` Telehealth) are
> reproduced **verbatim** in `[[Communication]]` **Part 2**, under a `SOURCE:` divider naming this
> file, so `[[GER5]] 0.x` still resolves by section name.
>
> **This file is kept rather than deleted because 15 wikilinks point at it.** The sourcing box
> above still applies to the moved content.
>
> `§0.2` and `§0.3` are marked `CF-PAIR` against `Communication §1.2` and `§1.8` — both copies kept
> in full, nothing reconciled. **Open disclosure and handover resolved to `[[EBM1]]` as declared
> owner**, which is what this file already said.
