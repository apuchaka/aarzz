...
**Worked example already in this project:** **D-dimer** is highly sensitive and poorly specific, which is exactly why it functions as a **rule-out** test in a patient with low pre-test probability of VTE and is useless as a rule-in test — see [[HemeOnc_03_Bleeding_Thrombosis_and_Transfusion]] Coagulation Screen and D-dimer Interpretation, which states the practical rule; this entry is the reasoning underneath it.
...
> [!note] Gap-filled from CSV ("Study design types and sources of bias," Public Health/Epidemiology category). **A second account is in this file at `§0.1 Study Design` (L14), which already declares the pair as `CF-PAIR`.** (This note previously reported a corpus-wide zero-hit search for *case-control*, *cohort study*, *selection bias*, *recall bias* and *confounding*; that search was run against the pre-merge corpus.) The file's previous "Evidence-based medicine — a brief note" placeholder explicitly recorded this as an unbuilt topic; this entry replaces it. Verified against the **NHMRC Designation of Levels of Evidence** (the Australian hierarchy in use since 1999, extended in 2009 to cover diagnostic, prognostic, aetiological and screening questions as well as treatment), Aug 2026.
