...
> [!note] **Continues `01_Cardiovascular` from `Cardio_11_VTE_Pulmonary_Hypertension_and_Vascular`.**
> That source block is split across `Cardio_08_ACS_IHD_Hypertension_and_Lipids` · `Cardio_09_ECG_and_Arrhythmias` · `Cardio_10_Valves_Endocarditis_Cardiomyopathy_HF_and_Pericardium` · `Cardio_11_VTE_Pulmonary_Hypertension_and_Vascular` · `Cardio_12_Congenital_and_Cardiac_Investigations`.
> The alias `01_Cardiovascular` resolves to `Cardio_09_ECG_and_Arrhythmias`, which holds its largest part.
...
> Here · `Endo_08_Diabetes_DKA_HHS_and_Metabolic_Syndrome §0.15.8 Austroads Driving Standards for Diabetes` ·
